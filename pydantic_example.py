...
class PatientSchema(BaseModel): # Step 1: Define a Pydantic model for patient data validation
    ...
    height: Annotated[float, Field(gt=0, strict=True)]  # height must be a positive float and strict type
    weight: Annotated[float, Field(gt=0, strict=True)]  # weight must be a positive float and strict type
    married : Optional[bool] = None
    gender:str = 'Male'
    allergies: Optional[List[str]] = Field(default=None, max_length=5) # by default all defined fields are required unless specified as Optional
    contact_details: Dict[str, str]
    address : Address = Field(title="Address of the patient", description="Address must include city, state, and zip code")
    ...
    @field_validator("email", mode="before") # Step 4: Custom validator for email field.
    @classmethod
    ...
    @model_validator(mode="after") # Step 5: Custom model-level validator to ensure age is realistic.
    @classmethod
...
print(patient1.model_dump(exclude={"address":{'state'}, 'married': True, 'allergies': True, 'name': True}, exclude_unset=True), type(patient1.model_dump()))  # Serialize the model format to a dictionary
print()
print(patient1.model_dump_json(include={'name': True, 'address': {'city'}, 'bmi': True}), type(patient1.model_dump_json()))  # Serialize the model format to a JSON string
